...
#ifdef CONFIG_ESD_CHECK_SECOND_REG
static int mdss_dsi_read_status2(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct dcs_cmd_req cmdreq;

	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = ctrl->status2_cmds.cmds;
	cmdreq.cmds_cnt = ctrl->status2_cmds.cmd_cnt;
	cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_RX;
	cmdreq.rlen = ctrl->status_cmds_rlen;
	cmdreq.cb = NULL;
	cmdreq.rbuf = ctrl->status_buf.data;
...
#ifdef CONFIG_ESD_CHECK_SECOND_REG
	if (ret == 1) {
		if (ctrl_pdata->status2_value == 0xffffffff)
			goto status2_read_exit;

		pr_debug("%s: read fisrt status reg value = 0x%x\n",
			    __func__, ctrl_pdata->status_buf.data[0]);
		if (ctrl_pdata->status2_cmds.link_state == DSI_HS_MODE)
			mdss_dsi_set_tx_power_mode(0, &ctrl_pdata->panel_data);
		...
		if (ret > 0) {
			if (ctrl_pdata->status_buf.data[0] !=
					ctrl_pdata->status2_value) {
				ret = -EINVAL;
			} else {
				ret = 1;
			}
		} else {
			pr_err("%s: read status2 failed !\n", __func__);
		}
	}
status2_read_exit: ;
#endif
