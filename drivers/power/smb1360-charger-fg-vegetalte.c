...
static int own_get_prop_batt_temp(struct device *dev)
{
	int rc = 0;
...
static ssize_t smb1360_battery_pmic_thermal_temp(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	int temp = own_get_prop_batt_temp(dev);

	return sprintf(buf, "%d\n", temp);
}

static int own_get_max_current(struct smb1360_chip *chip)
{
	int rc, own_current;
		u8 reg = 0;
	u8 chg_current;
...
static ssize_t smb1360_get_max_current(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	own_current = own_get_max_current(chip);
...
void own_set_current(struct smb1360_chip *chip, int current_limit)
{
	int i, rc;
	mutex_lock(&chip->current_change_lock);
	...
	if (i < 0) {
		pr_debug("own_set_current Couldn't find ICL mA rc=%d\n", rc);
		i = 0;
	}
	/* set input current limit */
	rc = smb1360_masked_write(chip, CFG_BATT_CHG_ICL_REG,
					INPUT_CURR_LIM_MASK, i);
	if (rc)
		pr_err("own_set_current Couldn't set ICL mA rc=%d\n", rc);

	pr_debug("own_set_current ICL set to = %d\n", input_current_limit[i]);

	mutex_unlock(&chip->current_change_lock);
}

static int backup_temp = 0;
void own_charging_adjust(struct smb1360_chip *chip)
{
	...
	if (default_max_current == 0){ // save default max current value
		default_max_current = own_get_max_current(chip);
	}
...
//if not charging ,return
	...
	board_temp = own_get_prop_batt_temp(chip->dev);
	if(board_temp <= 530){
	...
	max_current = own_get_max_current(chip);
	if(max_current <= 500){
		return;
	}
	...
	if(is_temp_rise){
		if(board_temp >=620){
			//limit to 600
			own_set_current(chip, 650);
		}else if(board_temp >=570){
			//limit to 700
			own_set_current(chip, 800);
		}else {
			//
			return;
		}
		...
		if(board_temp <=550){
			//set default max
			own_set_current(chip, default_max_current);
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,own-use-board-temp"))
	{
		chip->own_use_board_temp = true;
		pr_err("enabled own_use_board_temp for this project\n");
	}
