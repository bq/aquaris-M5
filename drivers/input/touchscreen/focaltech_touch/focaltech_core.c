...
/* request power_ldo_gpio gpio */
    if (gpio_is_valid(data->pdata->power_ldo_gpio))
    {
        err = gpio_request(data->pdata->power_ldo_gpio, "fts_power_ldo_gpio");
        if (err)
        {
            FTS_ERROR("[GPIO]power_ldo gpio request failed");
          //  goto err_power_ldo_gpio_dir;
        }

        err = gpio_direction_output(data->pdata->power_ldo_gpio, 1);
        if (err)
        {
            FTS_ERROR("[GPIO]set_direction for power_ldo gpio failed");
            goto err_power_ldo_gpio_dir;
        }
    }
#if 0
...
static int fts_power_source_ctrl(struct fts_ts_data *data, int enable)
{
    ...
    {
    	err =gpio_direction_output(data->pdata->power_ldo_gpio, 0);
	  if (err)
        {
            FTS_ERROR("[GPIO]set_direction for power_ldo gpio failed");
         //   goto err_reset_gpio_dir;
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->power_ldo_gpio = of_get_named_gpio(np, "focaltech,power_ldo-gpio",0);
...
static ssize_t class_fts_ts_info_show(struct class *class,
		struct class_attribute *attr, char *buf)
{

	memset(ts_info, 0, GTP_INFO_MAX_SIZE);
	fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &ts_version);
	snprintf(ts_info, GTP_INFO_MAX_SIZE,
			"FT5436,fw_version:%02X\n",
			ts_version);

	return snprintf(buf, GTP_INFO_MAX_SIZE, "%s\n", ts_info);
}

static CLASS_ATTR(ts_info, S_IRUSR, class_fts_ts_info_show, NULL);

static ssize_t class_fts_ts_version_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
...
static CLASS_ATTR(ts_version, S_IRUSR, class_fts_ts_version_show, NULL);

#if FTS_GESTURE_EN
static ssize_t fts_wakeup_gesture_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	int count;
	u8 val;
	...
	//fts_i2c_read_reg(fts_i2c_client, FTS_REG_GESTURE_EN, &val);
	//count = sprintf(buf, "Gesture Mode: %s\n", fts_gesture_data.mode ? "On" : "Off");
	val = fts_gesture_data.mode;
	count = sprintf(buf, "%d\n", val);
...
static ssize_t fts_wakeup_gesture_store(struct class *class, struct class_attribute *attr,
	   ...
	   mutex_lock(&fts_input_dev->mutex);
	    ...
	    if (!strncmp(buf,"1" , 1))
	    {
	        FTS_INFO("[GESTURE]enable gesture");
             //  fts_i2c_write_reg(fts_i2c_client, FTS_REG_GESTURE_EN, 0x01);
	        fts_gesture_data.mode = ENABLE;
	    }
	    else
	    {
	        FTS_INFO("[GESTURE]disable gesture");
		// fts_i2c_write_reg(fts_i2c_client, FTS_REG_GESTURE_EN, 0x00);
...
static CLASS_ATTR(en_gesture, S_IRWXUGO/*S_IWUSR | S_IRUSR*/, fts_wakeup_gesture_show,
			fts_wakeup_gesture_store);
#endif

static int gtp_device_info_init(struct i2c_client *client)
{
	int rc = 0;
	...
	if (rc < 0) {
		dev_err(&client->dev, "%s class create ts_info file failed!\n", __func__);
	...
	if (rc < 0) {
		dev_err(&client->dev, "%s class create ts_version file failed!\n", __func__);
		class_destroy(touchscreen_class);
		return rc;
	}
#if FTS_GESTURE_EN
	rc = class_create_file(touchscreen_class, &class_attr_en_gesture);
	if (rc < 0) {
		dev_err(&client->dev, "%s class create en_gesture file failed!\n", __func__);
		class_destroy(touchscreen_class);
		return rc;
	}
#endif

	fts_i2c_read_reg(client, FTS_REG_FW_VER, &ts_version);
	snprintf(ts_info, GTP_INFO_MAX_SIZE, "%s", "ft8736");
