...
static int cyttsp5_power_on(struct device *dev, bool on)
{
	...
	if (pdata->core_pdata->power_ldo >= 0) {
		rc = gpio_request(pdata->core_pdata->power_ldo, "cyttsp5-2v8_en");
		if (rc < 0) {
			printk("%s: request 2v8_en pin failed\n", __func__);
			goto cyttsp5_power_on_fail;
		}
		gpio_direction_output(pdata->core_pdata->power_ldo, 1);
	}

	pdata->core_pdata->vcc_i2c = regulator_get(dev, "vcc_i2c");
	if (IS_ERR(pdata->core_pdata->vcc_i2c)) {
		rc = PTR_ERR(pdata->core_pdata->vcc_i2c);
		printk("Regulator get failed rc=%d\n", rc);
		goto free_power_gpio;
	}
	if (regulator_count_voltages(pdata->core_pdata->vcc_i2c) > 0) {
		rc = regulator_set_voltage(pdata->core_pdata->vcc_i2c,
			1800000, 1800000);
		if (rc) {
	...
	rc = regulator_enable(pdata->core_pdata->vcc_i2c);
	if (rc) {
...
power_off:
	on_flag = 0;
	regulator_disable(pdata->core_pdata->vcc_i2c);
reg_vcc_i2c_put:
	regulator_put(pdata->core_pdata->vcc_i2c);
free_power_gpio:
	gpio_free(pdata->core_pdata->power_ldo);
	if (!on)
		return 0;
cyttsp5_power_on_fail:
	return -ENODEV;
}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	if(is_tp_driver_loaded) {
		dev_err(dev, "%s: other driver has been loaded\n",
					__func__);
		return -ENODEV;
	}
	...
	rc = cyttsp5_power_on(dev,1);
	if (rc) {
		dev_err(dev,"%s power on fail\n", __func__);
		goto power_on_fail;
	}
	...
	if (cd->cpdata->init) {
		...
		rc = cd->cpdata->init(cd->cpdata, 1, cd->dev); //cyttsp5_init()
	} else {
	...
	sprintf(tp_version, "[fw]V%d,[ic]CYPPRESS", cd->sysinfo.cydata.fw_ver_conf);
