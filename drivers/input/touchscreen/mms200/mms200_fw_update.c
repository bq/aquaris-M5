...
static int mms_flash_section(struct mms_ts_info *info, struct mms_fw_img *img, const u8 *data)
{
	...
	printk( "section [%d] update succeeded\n", img->type);
...
int mms_flash_fw(struct mms_ts_info *info,const u8 *data,int size, bool force)
{
	...
	if (retires < 0) {
		printk( "failed to obtain ver. info\n");
		isc_flag = false;
	...
	for (i = 0; i < fw_hdr->section_num; i++, offset += sizeof(struct mms_fw_img)) {
		...
		if (ver[img[i]->type] != target[i]) {
			...
			update_flag = true;
			printk("section [%d] is need to be updated. ver : 0x%x, bin : 0x%x\n",
			...
			printk("section [%d] is up to date\n", i);
		}
	}
	...
	printk("%s, after reboot\n", __func__);

	if (get_fw_version(client, ver)) {
		printk( "failed to obtain version after flash\n");
		ret = -1;
		goto out;
	} else {
		for (i = 0; i < fw_hdr->section_num; i++) {
			if (ver[img[i]->type] != target[i]) {
				printk("version mismatch after flash. [%d] 0x%x != 0x%x\n",
