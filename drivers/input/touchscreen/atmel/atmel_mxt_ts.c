...
//vcc_i2c maybe no need
	...
	{
		printk("power on %s, power_ldo_gpio\n", __func__);
...
static int mxt_power_init(struct mxt_data *data, bool on)
{
	...
	if (!on)
		goto pwr_deinit;
	if (gpio_is_valid(data->pdata->power_ldo_gpio))// ldo_gpio may the GPIO_24, it used to enable 3V3 power
	{
		printk("%s, power_ldo_gpio valid\n", __func__);
...
static CLASS_ATTR(en_gesture, S_IRWXUGO, mxt_plugin_wakeup_gesture_show,
...
static void update_keymap(struct mxt_platform_data *pdata)
{
	...
	pdata->keymap[T15_T97_KEY][2] = KEY_MENU;
        pdata->keymap[T93_KEY][0] = KEY_WAKEUP;
#endif
	return;
}
#endif
